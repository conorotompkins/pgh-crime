...
```{r load custom theme, echo = FALSE}
...
```{r lower case column names, echo = FALSE}
colnames(df) <- tolower(colnames(df))
```

```{r rename columnn names, echo = FALSE}
df <- df %>% 
...
```{r lubridate column names, echo = FALSE}
df <- df %>%
...
```{r reorder column names, echo = FALSE}
df <- df %>% 
  select(date, 
         year,
         month,
